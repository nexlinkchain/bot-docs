...
内联键盘显示在消息下方。每个按钮既可以打开一个 URL，也可以向你的机器人发送回调查询。通过 [sendMessage](METHODS.md#sendmessage) 及其他发送方法的 `reply_markup` 字段附加内联键盘。参见 [InlineKeyboardMarkup](TYPES.md#inlinekeyboardmarkup) 与 [InlineKeyboardButton](TYPES.md#inlinekeyboardbutton) 类型。
...
收到 [CallbackQuery](TYPES.md#callbackquery) 后，机器人应调用 [answerCallbackQuery](METHODS.md#answercallbackquery) 进行确认。
